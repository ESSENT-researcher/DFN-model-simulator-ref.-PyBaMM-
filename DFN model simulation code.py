...
def load_prada2013_params():
    """All parameters from Prada2013 for LFP/Graphite cell (2.3Ah)."""
    p = {}
    p["T"] = 298.15
    p["H"] = 0.6
    p["W"] = 0.3
    p["A_cell"] = p["H"] * p["W"]  # 0.18 m²
    ...
    p["R_p_n"] = 5.0e-6
    p["R_p_p"] = 5.0e-8   # 50nm LFP nanoparticles
    ...
    # Prada2013: sto_n_init = 0.81 (24750/30555), sto_p_init = 0.0038 (86.66/22806)
    p["c_s_init_n"] = 24749.55
    p["c_s_init_p"] = 86.6628
    ...
    # Kashkooli2017 for LFP: m_ref = 6e-7
    p["k_n"] = 6.48e-7
    p["k_p"] = 6.0e-7
...
def particle_rhs(c_s_2d, j_array, D_s, particle_mesh, a_s, c_s_max):
    ...
    for ix in range(N_x):
        ...
        grad_cs = G @ c_s          # (N_r-1,)
        flux_internal = -D_s * grad_cs  # (N_r-1,)
...
def compare_crates(material="NCM", crates=None, recipe_CA=None,
    ...
    for cr in crs_sorted:
        ...
        cap_delivered = t[-1] * p_out["Q_cell"] * cr / 3600.0
        delivered.append(cap_delivered)
    ...
    ax.set_ylabel("Delivered Capacity [Ah]")
    ax.set_title("Capacity vs C-rate")
